...
from config import CLAUDE_MODELS, GEMINI_MODELS, DEFAULT_MODEL
    ...
    with st.sidebar:
        ...
        if not offline_mode:
            ...
            st.markdown(
                ...
                f'還沒有 Key？→ '
                f'<a href="{key_link}" target="_blank" style="color:#2563EB;">{key_link_text}</a>'
                f' 取得</div>',
                unsafe_allow_html=True,
            )
        else:
        ...
        st.markdown("---")
        st.markdown(
            '<div style="font-size:11px;color:#9B9B9B;text-align:center;">'
            'AI Python Code Tutor v0.3.0<br>'
            'Class 解析 + Import 百科 v0.3.0</div>',
            unsafe_allow_html=True,
        )
