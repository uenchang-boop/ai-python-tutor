"""
可重複使用的 UI 元件模組 — 第 2 輪更新
新增：
  - render_function_card 支援 params_explain、return_explain、complexity badge
  - AI 來源標記（⚡快取 / 🤖 Gemini / 📖 離線）
"""
...
def render_header():
    ...
    <div style="margin-bottom:28px;padding-bottom:20px;border-bottom:1px solid #E8E5E0;">
        <div style="display:flex;align-items:baseline;gap:12px;flex-wrap:wrap;">
            ...
            <span class="tutor-version">v0.3.0</span>
        </div>
...
def render_welcome():
    ...
    <div class="welcome-wrap">
        ...
        <div style="margin-top:28px;font-size:12px;color:#9B9B9B;line-height:1.7;">
            💡 在左側欄輸入 Gemini API Key 啟用 AI 解釋<br>
            📖 無 Key 時自動使用離線關鍵字模板
        </div>
    </div>
    """, unsafe_allow_html=True)
...
def render_function_card(func: dict, explanation: dict, relations: dict):
    ...
    <div class="{card_class}">
        <div style="display:flex;justify-content:space-between;
                    align-items:flex-start;flex-wrap:wrap;gap:6px;margin-bottom:4px;">
            <div>{badge_html}</div>
            <div style="display:flex;gap:6px;align-items:center;">{comp_badge} {src_badge} {ret_html}</div>
        </div>
        {params_html}
        <div class="explain-block">
            <span class="explain-label label-def">🎯 功能說明</span>{definition}
        </div>
        <div class="explain-block guide-block">
            <span class="explain-label label-guide">💡 新手導讀</span>{guide}
        </div>
        {params_explain_html}
        {return_explain_html}
        {calls_html}
